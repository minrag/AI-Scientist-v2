...
- 做实验的时候 禁止使用合成数据,必须要使用已经存在的数据集
- 实验要求：
1定量实验部分:主要是通过充分实验的量化数据证明论文提出的算法是有效的，可信的，支撑论文的核心论点，与论文的主张形成逻辑闭环。
2消融研究部分:主要是通过更深入的实验数据证明论文提出的每一个创新组件都是必要的，量化每个创新组件对整体性能的影响和贡献程度
